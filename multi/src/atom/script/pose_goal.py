...
class robot_goal:
    ...
    def __init__(self) -> None:
        ...
        self.first_robot_x = 0.0
        self.first_robot_y = 0.0
        self.robot_yaw = 0.0
        self.atom_vel = rospy.Publisher('/atom2/cmd_vel', geometry_msgs.msg.Twist,queue_size=1)
        self.lifter = rospy.Publisher('/amazon_robot2/joint1_position_controller/command', Float64, queue_size=10)
        self.pose_robot_sub = rospy.Subscriber('/atom2/odom',Odometry,self.odom_callback)
        self.first_robot_sub = rospy.Subscriber('/odom',Odometry,self.odom_callback_first)
    ...
    def odom_callback_first(self,msg):
        self.first_robot_x = msg.pose.pose.position.x
        self.first_robot_y = msg.pose.pose.position.y
        self.first_robot_rot = msg.pose.pose.orientation
        self.first_robot_yaw = euler_from_quaternion([self.first_robot_rot.x,self.first_robot_rot.y,self.first_robot_rot.z,self.first_robot_rot.w])[2]
    ...
    def move_to_goal(self, goal, is_reverse):
        ...
        robot_one_position = (self.first_robot_x,self.first_robot_y)
        ...
        while math.dist(present_point, goal_point) > 0.05:
            ...
            fututre_point = (self.pose_robot_x + math.cos(present_angle), self.pose_robot_y + math.sin(present_angle))
            first_robot_angle = self.first_robot_yaw
            robot_one_position_next = (self.first_robot_x + math.cos(first_robot_angle), self.first_robot_y + math.sin(first_robot_angle))

            if math.dist(present_point, robot_one_position) < 1.2 and math.dist(fututre_point, robot_one_position) < 1:
                ...
                print("Collision - Robot1: ", robot_one_position, ' Robot2: ', present_point, ' Distance: ', math.dist(present_point, robot_one_position))
                print("Future Robot1", robot_one_position, ' Robot2: ', fututre_point, ' Distance: ', math.dist(robot_one_position, fututre_point))
            else:
