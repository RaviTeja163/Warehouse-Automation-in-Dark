...
class robot_goal:
    ...
    def move_to_goal(self, goal, is_reverse):
        ...
        while math.dist(present_point, goal_point) > 0.05:
            ...
            angle_correction = self.wrapTo2PI(goal_angle) - self.wrapTo2PI(present_angle)
            if abs(goal_angle) < 0.5 and abs(present_angle) < 0.5:
                angle_correction = goal_angle - present_angle
            elif abs(goal_angle) > 0.5 and abs(present_angle) < 0.5:
                angle_correction = self.wrapTo2PI(goal_angle) - present_angle
            elif abs(goal_angle) < 0.5 and abs(present_angle) > 0.5:
                angle_correction = goal_angle - self.wrapTo2PI(present_angle)
...
if __name__ == "__main__":
    ...
    try:
        ...
        path = [(4,0),(3,0),(2,0),(1,0),(0,0),(-1,0),(-1,0.9)]
        path1 = [(-1,-0.1),(1,0),(2,0),(3,0),(4,0),(4.9,0)]
        # atom_robot.move_to_goal([(3,0)], False)
